...
import { Layers, Mountain, Activity, ArrowDown, ArrowLeftRight } from 'lucide-react';
    ...
    <div className="w-full h-full bg-stone-100 flex flex-col p-6 relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-stone-200">
          <span className="text-sm text-stone-500 mr-2">当前构造:</span>
          <span className="font-bold text-stone-800">
            {structureType === 'normal' ? '水平岩层' : 
      ...
      {/* 控制面板 */}
      <div className="mt-8 bg-white p-6 rounded-xl shadow-lg border border-stone-200 flex gap-8 items-end">
        <div className="flex-1 space-y-4">
          <label className="text-sm font-bold text-stone-700 flex items-center gap-2">
            <Activity size={16} /> 作用力类型
          </label>
          <div className="flex gap-2">
            <button
              onClick={() => setForceType('pressure')}
              className={`flex-1 py-2 rounded-lg border-2 transition-all ${
                forceType === 'pressure' ? 'border-red-500 bg-red-50 text-red-700' : 'border-stone-200 hover:border-red-300'
              }`}
            >
              {t('geography.pressure')} (挤压)
            </button>
            <button
        ...
        <button
          onClick={handleReset}
          className="px-6 py-2 bg-stone-200 hover:bg-stone-300 text-stone-700 rounded-lg font-bold transition-colors"
        >
          {t('lab.reset')}
        </button>
      </div>
